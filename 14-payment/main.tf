module "payment" {
  ...
  #listener rule
  # Adding rule to private ALB listener i.e. if any request payment.app.devidevops.online hits then request should be forward to user target group
  alb_listener_arn = data.aws_ssm_parameter.app_alb_listener_arn.value
  rule_priority = 50
  host_header = "payment.app.devidevops.online"
