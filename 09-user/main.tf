module "user" {
  ...
  #listener rule
  # Adding rule to private ALB listener i.e. if any request user.app.devidevops.online hits then request should be forward to user target group
  alb_listener_arn = data.aws_ssm_parameter.app_alb_listener_arn.value
  rule_priority = 20 #catalogue have 10 already
  host_header = "user.app.devidevops.online"
